...
class AjaxFormMixin:
    """Mixin for CreateView/UpdateView: returns JSON on AJAX POST, renders
    just the form HTML on AJAX GET (so the modal can inject it)."""
    ...
    def form_invalid(self, form):
        if self._is_ajax():
            return JsonResponse({'success': False, 'form_html': self._form_html(form)})
...
class DashboardView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        gammes_qs = GammeControle.objects.all()
        epis_qs = epi.objects.all()
        moyens_qs = moyens_controle.objects.all()

        total_missions = missions_qs.count()
        active_missions = missions_qs.filter(statut=True).count()
        inactive_missions = total_missions - active_missions
        missions_with_pdf = missions_qs.filter(pdf_file__isnull=False).exclude(pdf_file='').count()
        total_gammes = gammes_qs.count()
        active_gammes = gammes_qs.filter(statut=True).count()
        total_epis = epis_qs.count()
        total_moyens = moyens_qs.count()

        recent_missions = missions_qs.order_by('-date_creation')[:5]
        recent_gammes = gammes_qs.select_related('mission').order_by('-date_creation')[:5]

        context = {
            'total_missions': total_missions,
            'active_missions': active_missions,
            'inactive_missions': inactive_missions,
            'missions_with_pdf': missions_with_pdf,
            'total_gammes': total_gammes,
            'active_gammes': active_gammes,
            'total_epis': total_epis,
            'total_moyens': total_moyens,
...
class GammeControleListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['active_gammes'] = GammeControle.objects.filter(statut=True).count()
        return context
...
class MissionControleListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        stats_queryset = MissionControle.objects.all()
        if self.request.user.is_op:
            stats_queryset = stats_queryset.filter(statut=True)
        ...
        context['missions_total'] = stats_queryset.count()
        context['missions_active'] = stats_queryset.filter(statut=True).count()
        context['missions_inactive'] = stats_queryset.filter(statut=False).count()
        context['missions_with_pdf'] = stats_queryset.exclude(pdf_file='').exclude(pdf_file__isnull=True).count()
...
class EpiCreateView(AjaxFormMixin, LoginRequiredMixin, UserPassesTestMixin, CreateView):
    def test_func(self):
        # Allow access if user is admin, responsable, or ro
        user = self.request.user
        if not user.is_authenticated:
...
class EpiUpdateView(AjaxFormMixin, LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    def test_func(self):
        # Allow access if user is admin, responsable, or ro
        user = self.request.user
        if not user.is_authenticated:
...
class UserListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['user_count'] = qs.count()
        context['users_op'] = qs.filter(is_op=True).count()
        context['users_rs'] = qs.filter(is_rs=True).count()
        context['users_ro'] = qs.filter(is_ro=True).count()
        context['users_inactive'] = qs.filter(last_login__isnull=True).count()
        return context

class UserUpdateView(AjaxFormMixin, LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    def test_func(self):
        # Allow access if user is admin, responsable, or ro
        user = self.request.user
        if not user.is_authenticated:
...
class logoutView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        return redirect('Gamme:demo_select')
...
# ================================================================
# DEMO MODE — Portfolio demonstration
# These views allow instant role-based login without passwords.
# To reactivate real auth: change the default URL redirect back to
# 'Gamme:login' in urls.py and remove/hide these routes.
# ================================================================

DEMO_USERS = {
    'admin': {
        'username': 'demo_admin',
        'email': 'demo@abserve.fr',
        'first_name': 'Admin',
        'last_name': 'Demo',
        'is_superuser': True,
        'is_staff': True,
...
def demo_select_view(request):
    if request.user.is_authenticated:
        return redirect('Gamme:missioncontrole_list')
    return render(request, 'gamme/demo_select.html')
...
def demo_login_view(request, role):
    ...
    if user.is_op:
        return redirect('Gamme:operateur_dashboard')
    return redirect('Gamme:missioncontrole_list')
...
class ajouter_utilisateur(AjaxFormMixin, LoginRequiredMixin, CreateView):
    model = User
    form_class = RegisterForm
...
class MoyenControleCreateView(AjaxFormMixin, LoginRequiredMixin, UserPassesTestMixin, CreateView):
    def test_func(self):
        # Allow access if user is admin, responsable, or ro
        user = self.request.user
        if not user.is_authenticated:
...
class MoyenControleUpdateView(AjaxFormMixin, LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    def test_func(self):
        # Allow access if user is admin, responsable, or ro
        user = self.request.user
        if not user.is_authenticated:
